[![Build Status](https://travis-ci.org/mishal/GeoPHP.svg?branch=master)](https://travis-ci.org/mishal/GeoPHP)
 ...
 * The latest stable version can always be downloaded at: <https://github.com/phayes/GeoPHP/releases>
...
// Polygon WKT example
$polygon = \GeoPHP\Geo::load('POLYGON((1 1,5 1,5 5,1 5,1 1),(2 2,2 3,3 3,3 2,2 2))','wkt');
...
$multipoint = \GeoPHP\Geo::load($json, 'json');
$multipointPoints = $multipoint->getComponents();
$first_wkt = $multipointPoints[0]->out('wkt');
...
GeoPHP (can be wkt or wkb, but must be the same as how you extracted it from your database):
    ...
    $geom = \GeoPHP\Geo::load($dbRow,'wkt');

You can collect multiple geometries into one (note that you must use wkt for this):

    $geom = \GeoPHP\Geo::load("GEOMETRYCOLLECTION(".$dbString1.",".$dbString2.")",'wkt');

Calling get components returns the sub-geometries within a geometry as an array.

    $geom2 = \GeoPHP\Geo::load("GEOMETRYCOLLECTION(LINESTRING(1 1,5 1,5 5,1 5,1 1),LINESTRING(2 2,2 3,3 3,3 2,2 2))");
...
$result = pg_fetch_all(pg_query($connection, "SELECT ST_asBinary($column) as geom FROM $table"));
foreach ($result as $item) {
  $wkb = pg_unescape_bytea($item['geom']); // Make sure to unescape the hex blob
  $geom = \GeoPHP\Geo::load($wkb, 'ewkb'); // We now a full GeoPHP Geometry object
  
  // Let's insert it back into the database
  $insert_string = pg_escape_bytea($geom->out('ewkb'));
  pg_query($connection, "INSERT INTO $table ($column) values (ST_GeomFromWKB('$insert_string'))");
}
...
foreach ($result as $item) {
  ...
  $geom = \GeoPHP\Geo::load($wkb, 'ewkb'); // We now have a GeoPHP Geometry
